...
func (s *Server) adminRoutes(r *echo.Group) {
	...
	{
		handler := ssehandler.NewHandler()

		sseRoute.GET("", handler.Listen)
	}
	...
	accountManagementRoute := r.Group("/account-management")
	{
		...
		accountManagementRoute.POST("/accounts/suspend/:adminId", handler.SuspendAccount)
	...
	settingRoute := r.Group("/settings")
	{
		...
		settingRoute.POST("/weights", handler.ConfigureWeights)
	}
	...
	logsRoute := r.Group("/logs")
	{
		...
		logsRoute.Use(middleware.EnsureAdmin)
